saveDiagnostics <- function(eval_data, output_FileName) {  # Save the model diagnostics
  ...
  whisker.render(template_1 %>% str_remove_all('\n') %>% str_replace_all('  ', ' '), render_data ) %>% cat(file = file.path(process_output_dir, output_FileName))
